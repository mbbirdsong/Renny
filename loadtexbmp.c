...
// for my local dev, should be fine if not there
#ifdef WINDOWS
char* dir =".\\textures\\";
#else
char* dir ="/textures/";
...
unsigned int LoadTexBMP(char* file)
{
   ...
   //  Open file (assume local dir)
   f = fopen(file,"rb");
   if (!f)
   {
       // try with MBB dev dir value
	   strcpy(dir_n_file, app_launch);
       strcat(dir_n_file, dir);
       strcat(dir_n_file, file);
       f = fopen(dir_n_file,"rb");
